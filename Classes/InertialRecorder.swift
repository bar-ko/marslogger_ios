...
class NodeWrapper: NSObject {
    ...
    func compare(_ otherObject: NodeWrapper) -> ComparisonResult {
        return NSNumber(value: self.time).compare(NSNumber(value: otherObject.time))
    }
}
...
func createOutputFolderURL() -> URL? {
    ...
    let paths = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask)
    guard let documentsURL = paths.last else { return nil }
    let outputFolderURL = documentsURL.appendingPathComponent(dateTimeString, isDirectory: true)
...
class InertialRecorder: NSObject, CLLocationManagerDelegate {
    ...
    private func interpolateGPSData(atTime targetTime: TimeInterval, 
                                   fromArray gpsArray: [GPSNodeWrapper], 
                                   currentIndex: inout Int) -> GPSNodeWrapper? {
        ...
        if let lowerGPS = lowerGPS, let upperGPS = upperGPS, lowerIndex >= 0, upperIndex >= 0 {
    ...
    private func interpolate(accelGyroData: [NodeWrapper], 
        ...
        let sortedArrayGyro = gyroArray.sorted { $0.compare($1) == .orderedAscending }
        let sortedArrayAccel = accelArray.sorted { $0.compare($1) == .orderedAscending }
        ...
        let sortedGPSArray = gpsData.sorted { $0.compare($1) == .orderedAscending }
        
        print("Interpolating data: \(mutableGyroCopy.count) gyro samples, \(mutableAccelCopy.count) accel samples, \(sortedGPSArray.count) GPS samples")
        
        // interpolate
        var mainString = ""
        var accelIndex = 0
        var gpsIndex = 0
        mainString += "Timestamp[nanosec], gx[rad/s], gy[rad/s], gz[rad/s], ax[m/s^2], ay[m/s^2], az[m/s^2], latitude[deg], longitude[deg], speed[m/s]\n"
        ...
        for gyroIndex in 0..<mutableGyroCopy.count {
            ...
            let nwGPS = interpolateGPSData(atTime: nwg.time, fromArray: sortedGPSArray, currentIndex: &gpsIndex)
            
            if nwg.time < nwa.time {
                continue
            } else if nwg.time == nwa.time {
                if let nwGPS = nwGPS {
                    mainString += String(format: "%@, %.15f, %.15f, %.15f, %.15f, %.15f, %.15f, %.15f, %.15f, %.15f\n",
                ...
                if upperIndex == lowerIndex {
                    ...
                    guard lowerIndex >= 0 && lowerIndex < mutableAccelCopy.count &&
                          upperIndex >= 0 && upperIndex < mutableAccelCopy.count else {
                        break
                    }
                    ...
                    print("Impossible lower and upper bound \(lowerIndex) \(upperIndex) for gyro timestamp \(String(format: "%.5f", nwg.time))")
                }
    ...
    func switchRecording() {
        if isRecording {
            ...
            if !interpolateAccel {
                mainString += "Timestamp[nanosec], x, y, z[(a:m/s^2)/(g:rad/s)], isGyro?\n"
                if let rawData = rawAccelGyroData {
                    for nw in rawData {
                        mainString += String(format: "%.7f, %.5f, %.5f, %.5f, %d\n", nw.time, nw.x, nw.y, nw.z, nw.isGyro ? 1 : 0)
                    }
                }
            } else {
                // linearly interpolate acceleration offline
                if let rawAccelGyro = rawAccelGyroData, let rawGPS = rawGPSData, let startTime = timeStartImu {
                    mainString = interpolate(accelGyroData: rawAccelGyro, gpsData: rawGPS, startTime: startTime)
                }
            }
            ...
            if let fileURL = fileURL {
                do {
                    try mainString.write(to: fileURL, atomically: true, encoding: .utf8)
            ...
            bootTimeReference = Date().timeIntervalSince1970 - ProcessInfo.processInfo.systemUptime
            gpsUpdateCount = 0
            lastGPSUpdateTime = 0
            print("Boot time reference: \(String(format: "%.6f", bootTimeReference))")
    ...
    func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
        ...
        guard let location = locations.last,
              ...
              location.coordinate.longitude != 0 else {
            return
        }
        ...
        gpsNode.time = location.timestamp.timeIntervalSince1970 - bootTimeReference
        ...
        if gpsUpdateCount % 10 == 0 {
            let timeSinceLastUpdate = lastGPSUpdateTime > 0 ? gpsNode.time - lastGPSUpdateTime : 0
            let gpsFrequency = lastGPSUpdateTime > 0 ? 1.0 / timeSinceLastUpdate : 0
            print("GPS Update #\(gpsUpdateCount): lat=\(String(format: "%.15f", gpsNode.latitude)), lon=\(String(format: "%.15f", gpsNode.longitude)), speed=\(String(format: "%.15f", gpsNode.speed)), time=\(String(format: "%.6f", gpsNode.time)), freq=\(String(format: "%.1f", gpsFrequency)) Hz")
        } else {
            print("GPS: lat=\(String(format: "%.15f", gpsNode.latitude)), lon=\(String(format: "%.15f", gpsNode.longitude)), speed=\(String(format: "%.15f", gpsNode.speed)), time=\(String(format: "%.6f", gpsNode.time))")
        }
    ...
    func locationManager(_ manager: CLLocationManager, didFailWithError error: Error) {
        print("GPS location manager failed with error: \(error.localizedDescription)")
    }
}
